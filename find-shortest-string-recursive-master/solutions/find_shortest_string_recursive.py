...
def find_shortest_string_recursive(lst):
    if len(lst) == 1:
        return lst[0]

    result = find_shortest_string_recursive(lst[1:])

    return lst[0] if len(lst[0]) <= len(result) else result

strings = ['aaa', 'a', 'bb', 'ccc']
print("Expecting: 'a'")
print(find_shortest_string_recursive(strings))
...
def find_shortest_string_recursive(list):
    """
    The function `find_shortest_string_recursive` recursively finds the shortest string in a given list
    of strings.
    
    :param list: The parameter `list` in the `find_shortest_string_recursive` function is a list of
    strings
    :return: The function `find_shortest_string_recursive` returns the shortest string from the given
    list of strings.
    """
